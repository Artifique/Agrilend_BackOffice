...
import { 
  ...
  Eye, Activity, Database, Key, DollarSign, Truck,
  Users, Settings, BarChart3, RefreshCw
} from 'lucide-react';
...
interface AuditLog {
  ...
  action: string;
  category: 'authentication' | 'transaction' | 'user_management' | 'system' | 'security' | 'financial' | 'logistics';
...
const mockAuditLogs: AuditLog[] = [
  ...
  },
  {
    ...
    details: {
      ...
      location: 'Abidjan, Côte d\'Ivoire',
      changes: [
        { field: 'amount', oldValue: null, newValue: 125.50 },
  ...
  },
  {
    ...
    details: {
      changes: [
        ...
        { field: 'lastRotation', oldValue: '2023-10-15T00:00:00Z', newValue: '2024-01-15T14:15:30Z' }
      ],
  ...
  },
  {
    ...
    details: {
      ...
      userAgent: 'Mozilla/5.0 (Android 11; Mobile; rv:78.0) Gecko/78.0 Firefox/78.0',
      location: 'Yamoussoukro, Côte d\'Ivoire',
      changes: [
        { field: 'status', oldValue: 'in_transit', newValue: 'delivered' },
        { field: 'deliveredAt', oldValue: null, newValue: '2024-01-15T14:05:00Z' }
      ],
      metadata: { driver: 'Driver001', vehicle: 'VEH001', gpsLat: 6.8276, gpsLng: -5.2893 }
    },
  ...
  },
  {
    ...
    details: {
      ...
      metadata: { reason: 'market_adjustment', effectiveDate: '2024-01-16T00:00:00Z' }
    },
    sessionId: 'sess_abc123',
...
const AuditLogs: React.FC = () => {
  ...
  const filteredLogs = useMemo(() => {
    return auditLogs.filter(log => {
      ...
      return matchesSearch && matchesCategory && matchesSeverity && matchesStatus && matchesUserType && matchesDate;
    });
  }, [auditLogs, searchTerm, categoryFilter, severityFilter, statusFilter, userTypeFilter, dateRange]);
  ...
  const getSeverityBadge = (severity: string) => {
    const severityConfig = {
      low: { color: 'bg-green-100 text-green-800', icon: CheckCircle, text: 'Faible' },
      medium: { color: 'bg-yellow-100 text-yellow-800', icon: AlertTriangle, text: 'Moyen' },
      high: { color: 'bg-orange-100 text-orange-800', icon: AlertTriangle, text: 'Élevé' },
      critical: { color: 'bg-red-100 text-red-800', icon: AlertTriangle, text: 'Critique' }
    };
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      success: { color: 'bg-green-100 text-green-800', icon: CheckCircle, text: 'Succès' },
      failure: { color: 'bg-red-100 text-red-800', icon: XCircle, text: 'Échec' },
      warning: { color: 'bg-yellow-100 text-yellow-800', icon: AlertTriangle, text: 'Avertissement' },
      info: { color: 'bg-blue-100 text-blue-800', icon: Activity, text: 'Info' }
    };
      ...
      cell: ({ row }) => (
        <div className="text-sm">
          <div className="font-medium">
            {new Date(row.getValue('timestamp')).toLocaleDateString('fr-FR')}
          </div>
          <div className="text-gray-500">
            {new Date(row.getValue('timestamp')).toLocaleTimeString('fr-FR')}
          </div>
        </div>
      )
    },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            ...
            <div className="text-sm text-gray-500">{getUserTypeLabel(log.userType)}</div>
          </div>
        );
      }
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Journalisation et Traçabilité</h1>
          <p className="text-gray-600 mt-1">Logs d'audit et traçabilité complète des actions</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Transactions BC</p>
              <p className="text-2xl font-bold text-green-600">{stats.blockchainTxs}</p>
            </div>
      ...
      >
        {selectedLog && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-600">ID Log:</span>
                    <span className="font-mono text-sm">{selectedLog.id}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-600">Timestamp:</span>
                    <span className="text-sm">{new Date(selectedLog.timestamp).toLocaleString('fr-FR')}</span>
                  </div>
                  <div className="flex items-center justify-between">
              ...
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-gray-900">Utilisateur et Statut</h3>
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
            ...
            {/* Ressource */}
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Ressource</h3>
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <div className="text-sm font-medium text-gray-600 mb-1">Ressource</div>
                    <div className="text-sm font-medium">{selectedLog.resource}</div>
                  </div>
                  {selectedLog.resourceId && (
                    <div>
                      <div className="text-sm font-medium text-gray-600 mb-1">ID Ressource</div>
                      <div className="text-sm font-mono">{selectedLog.resourceId}</div>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div>
              ...
              <div className="space-y-4">
                {selectedLog.details.ipAddress && (
                  <div className="bg-gray-50 rounded-lg p-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        <div className="text-sm font-medium text-gray-600 mb-1">Adresse IP</div>
                        <div className="text-sm font-mono">{selectedLog.details.ipAddress}</div>
                      </div>
                      {selectedLog.details.location && (
                        <div>
                          <div className="text-sm font-medium text-gray-600 mb-1">Localisation</div>
                          <div className="text-sm">{selectedLog.details.location}</div>
                        </div>
                      )}
                      {selectedLog.sessionId && (
                        <div>
                          <div className="text-sm font-medium text-gray-600 mb-1">Session ID</div>
                ...
                {selectedLog.details.userAgent && (
                  <div className="bg-gray-50 rounded-lg p-4">
                    <div className="text-sm font-medium text-gray-600 mb-1">User Agent</div>
                    <div className="text-xs font-mono break-all">{selectedLog.details.userAgent}</div>
                  </div>
                )}
              </div>
            </div>
            ...
            {selectedLog.details.changes && selectedLog.details.changes.length > 0 && (
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Changements</h3>
                  ...
                  {selectedLog.details.changes.map((change, index) => (
                    <div key={index} className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
                        <div>
                          <div className="text-gray-600">Ancienne valeur:</div>
                          <div className="font-mono">{JSON.stringify(change.oldValue)}</div>
                        </div>
                        <div>
                          <div className="text-gray-600">Nouvelle valeur:</div>
                          <div className="font-mono">{JSON.stringify(change.newValue)}</div>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            {(selectedLog.blockchainTxId || selectedLog.hederaAccountId) && (
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Blockchain</h3>
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {selectedLog.blockchainTxId && (
                      <div>
                        <div className="text-sm font-medium text-green-800 mb-1">Transaction ID</div>
                        <div className="text-sm font-mono">{selectedLog.blockchainTxId}</div>
                      </div>
                    )}
                    {selectedLog.hederaAccountId && (
                      <div>
                        <div className="text-sm font-medium text-green-800 mb-1">Account Hedera</div>
                        <div className="text-sm font-mono">{selectedLog.hederaAccountId}</div>
                      </div>
                    )}
                  </div>
                </div>
