...
const HederaKeysManagement: React.FC = () => {
  const [hederaKeys, setHederaKeys] = useState<HederaKey[]>([]);
  ...
  const handleBackupFormChange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>
  ) => {
    const { name, value } = e.target;
    setBackupFormData((prev) => ({ ...prev, [name]: value }));
  };

  // Function to simulate fetching Hedera keys from an API
  const fetchHederaKeys = async () => {
    setIsLoading(true);
    ...
    setHederaKeys(mockHederaKeys);
    setIsLoading(false);
  };

  useEffect(() => {
    fetchHederaKeys();
  }, []); // Fetch Hedera keys on component mount

  // Filtrage des données
  const filteredKeys = useMemo(() => {
    return hederaKeys.filter((key) => {
      const matchesSearch =
        key.accountId.includes(searchTerm) ||
      ...
      const matchesSecurity =
        securityFilter === "all" || key.securityLevel === securityFilter;
  ...
  const handleGenerateSubmit = async () => {
    ...
    try {
      ...
      // Example: await fetch(`/api/hedera-keys/generate`, {
      //   method: 'POST',
      //   headers: { 'Content-Type': 'application/json' },
      //   body: JSON.stringify(generateFormData),
      // });
      await new Promise((resolve) => setTimeout(resolve, 500));
      console.log("Generating key:", generateFormData);
      alert("Clé Hedera générée avec succès !");
      ...
      fetchHederaKeys(); // Refresh keys list
    } catch (error) {
  ...
  const confirmRotateKey = async () => {
    if (selectedKey) {
      setIsLoading(true);
      try {
        // TODO: Replace with actual API call to rotate key
        // Example: await fetch(`/api/hedera-keys/${selectedKey.id}/rotate`, {
        //   method: 'PUT',
        //   headers: { 'Content-Type': 'application/json' },
        // });
        await new Promise((resolve) => setTimeout(resolve, 500));
        alert("Clé Hedera tournée avec succès !");
        setShowRotateModal(false);
        setSelectedKey(null);
        fetchHederaKeys(); // Refresh keys list
      } catch (error) {
        console.error("Error rotating key:", error);
        alert("Erreur lors de la rotation de la clé.");
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const confirmBackupKey = async () => {
    if (selectedKey) {
      setIsLoading(true);
      try {
        // TODO: Replace with actual API call to backup key
        // Example: await fetch(`/api/hedera-keys/${selectedKey.id}/backup`, {
        //   method: 'PUT',
        //   headers: { 'Content-Type': 'application/json' },
        //   body: JSON.stringify(backupFormData),
        // });
        await new Promise((resolve) => setTimeout(resolve, 500));
        alert("Clé Hedera sauvegardée avec succès !");
        setShowBackupModal(false);
        setSelectedKey(null);
        setBackupFormData({ backupLocation: "" });
        fetchHederaKeys(); // Refresh keys list
      } catch (error) {
  ...
  const handleExport = () => {
    const data = filteredKeys.map((key) => ({
      ...
      "Prochaine rotation": key.nextRotation
        ? new Date(key.nextRotation).toLocaleDateString("fr-FR")
        : "N/A",
    }));
  ...
  const columns: ColumnDef<HederaKey>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Badge className="bg-blue-100 text-blue-800">
          {getTypeLabel(row.original.keyType)}
        </Badge>
      ),
    },
    {
    ...
    },
    {
      ...
      header: "Sécurité",
      cell: ({ row }) => getSecurityBadge(row.original.securityLevel),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="text-sm">
            {lastUsed
              ? new Date(lastUsed).toLocaleDateString("fr-FR")
              : "Jamais"}
          </div>
        );
      },
  ...
  const stats = useMemo(() => {
    ...
    const compromised = hederaKeys.filter(
      (k) => k.status === "compromised"
    ).length;
    const critical = hederaKeys.filter(
      (k) => k.securityLevel === "critical"
    ).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">
            Gestion des Clés Hedera
          </h1>
          <p className="text-gray-600 mt-1">
            Sécurité et gestion des clés blockchain
          </p>
        </div>
      ...
      {/* Tableau */}
      <Card className="p-6">
        <DataTable data={paginatedKeys} columns={columns} />
        {totalPages > 1 && (
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        copyToClipboard(selectedKey.privateKey || "")
                      }
                      className="h-8 w-8 p-0"
                    >
                      <Copy className="h-4 w-4" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {selectedKey.nextRotation && (
                    <div className="bg-gray-50 rounded-lg p-4">
                      ...
                      <div className="text-sm">
                        {new Date(selectedKey.nextRotation).toLocaleDateString(
                          "fr-FR"
                        )}
                      </div>
                    </div>
        ...
        <div className="space-y-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-medium text-blue-800">
                Génération de Clé Hedera
              </span>
            </div>
            <p className="text-sm text-blue-700">
              Une nouvelle paire de clés sera générée selon les standards de
              sécurité Hedera. La clé privée sera chiffrée et stockée de manière
              sécurisée.
            </p>
          </div>
          ...
          <div className="space-y-4">
            <FormField
              ...
              name="keyType"
              type="select"
              value={generateFormData.keyType}
            ...
            <FormField
              ...
              name="ownerId"
              type="select"
              value={generateFormData.ownerId}
              onChange={handleGenerateFormChange}
              options={[
                { value: "", label: "Sélectionner un propriétaire..." },
                { value: "F001", label: "Jean Kouassi (Agriculteur)" },
                { value: "B001", label: "Marie Traoré (Acheteur)" },
                { value: "L001", label: "Transport Express SARL (Logistique)" },
                { value: "PLATFORM", label: "AGRILEND Platform" },
              ]}
            />
            ...
            <FormField
              ...
              name="securityLevel"
              type="select"
              value={generateFormData.securityLevel}
              onChange={handleGenerateFormChange}
              options={[
                { value: "medium", label: "Moyen" },
                { value: "high", label: "Élevé" },
            ...
            <div>
              ...
              <div className="space-y-2">
                {["transfer", "tokenize", "stake", "freeze"].map(
                  (permission) => (
                    <label key={permission} className="flex items-center">
                      <input
                        ...
                        name="permissions"
                        value={permission}
                        checked={generateFormData.permissions.includes(
                          permission
                        )}
                        onChange={handleGenerateFormChange}
                      />
          ...
          <div className="space-y-4">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              ...
              <p className="text-sm text-yellow-700">
                Vous êtes sur le point de désactiver la clé{" "}
                <strong>{selectedKey.accountId}</strong>. Une nouvelle clé sera
          ...
          <div className="space-y-4">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <p className="text-sm text-green-700">
                La clé sera sauvegardée dans un coffre-fort sécurisé avec
            ...
            <FormField
              label="Localisation de sauvegarde"
              name="backupLocation"
              type="text"
