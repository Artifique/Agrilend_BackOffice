...
import { Truck, CheckCircle, XCircle, Clock, Package, User, Calendar, Navigation, Building2, Phone, Mail } from 'lucide-react';
...
const DeliveryActions: React.FC<DeliveryActionsProps> = ({ delivery, onStart, onComplete, onCancel }) => {
    ...
    <div className="space-y-4">
      <div className="bg-gray-50 p-4 rounded-lg">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <h3 className="font-semibold text-gray-700 mb-2">Détails de la livraison</h3>
...
const PartnerActions: React.FC<PartnerActionsProps> = ({ partner, onActivate }) => {
...
const LogisticsManagementOptimized: React.FC = () => {
  ...
  const deliveriesData: Delivery[] = useMemo(() => [
    {
      id: 1,
      orderId: 1001,
      product: 'Tomates',
  ...
  const partnersData: LogisticsPartner[] = useMemo(() => [
    {
      id: 1,
        ...
        cell: (info) => (
          <div className="flex items-center">
            ...
            <div>
              ...
              <div className="text-sm text-gray-500">{info.row.original.product}</div>
            </div>
          </div>
        ),
        enableSorting: true,
          ...
          return (
            <div className="flex flex-wrap gap-1">
              {services.map((service: string, index: number) => (
                <span 
                  key={index}
                  className={`px-2 py-1 rounded-full text-xs font-medium ${serviceColors[service as keyof typeof serviceColors] || 'bg-gray-100 text-gray-800'}`}
                >
                  {service}
                </span>
              ))}
  ...
  const deliveryFormFields = useMemo(() => [
    ...
    { name: 'product', label: 'Produit', type: 'text' as const, placeholder: 'Tomates', required: true },
    { name: 'farmer', label: 'Agriculteur', type: 'text' as const, placeholder: 'Jean Kouassi', required: true },
    { name: 'buyer', label: 'Acheteur', type: 'text' as const, placeholder: 'Restaurant Le Gourmet', required: true },
    { name: 'quantity', label: 'Quantité (kg)', type: 'number' as const, placeholder: '50', required: true, min: 1 },
    { name: 'pickupLocation', label: 'Lieu de ramassage', type: 'text' as const, placeholder: 'Ferme Kouassi, Abidjan', required: true },
    { name: 'deliveryLocation', label: 'Lieu de livraison', type: 'text' as const, placeholder: 'Restaurant Le Gourmet, Plateau', required: true },
    { name: 'scheduledDate', label: 'Date prévue', type: 'date' as const, required: true },
    { name: 'driver', label: 'Chauffeur', type: 'text' as const, placeholder: 'Moussa Traoré', required: true },
    { name: 'vehicle', label: 'Véhicule', type: 'text' as const, placeholder: 'Camion réfrigéré #TR001', required: true },
    { name: 'estimatedDuration', label: 'Durée estimée (heures)', type: 'number' as const, placeholder: '2', required: true, min: 1 },
    { name: 'distance', label: 'Distance (km)', type: 'number' as const, placeholder: '15', required: true, min: 1 },
    { name: 'cost', label: 'Coût (FCFA)', type: 'number' as const, placeholder: '25000', required: true, min: 1 },
    { name: 'notes', label: 'Notes', type: 'textarea' as const, placeholder: 'Notes sur la livraison...', rows: 3 }
  ], []);
  ...
  const deliveryValidationRules = useMemo(() => ({
    orderId: (value: unknown) => {
      ...
      if (!num || num < 1) return 'L\'ID de commande doit être un nombre positif';
      return null;
    },
    product: (value: unknown) => {
      const str = String(value);
      if (!str.trim()) return 'Le produit est requis';
      if (str.length < 2) return 'Le produit doit contenir au moins 2 caractères';
      return null;
    },
    farmer: (value: unknown) => {
      const str = String(value);
      if (!str.trim()) return 'L\'agriculteur est requis';
      if (str.length < 2) return 'L\'agriculteur doit contenir au moins 2 caractères';
      return null;
    },
    buyer: (value: unknown) => {
      const str = String(value);
      if (!str.trim()) return 'L\'acheteur est requis';
      if (str.length < 2) return 'L\'acheteur doit contenir au moins 2 caractères';
      return null;
    },
    ...
    pickupLocation: (value: unknown) => {
      ...
      if (str.length < 3) return 'Le lieu de ramassage doit contenir au moins 3 caractères';
      return null;
    },
    deliveryLocation: (value: unknown) => {
      const str = String(value);
      if (!str.trim()) return 'Le lieu de livraison est requis';
      if (str.length < 3) return 'Le lieu de livraison doit contenir au moins 3 caractères';
      return null;
    },
    ...
    driver: (value: unknown) => {
      ...
      if (str.length < 2) return 'Le chauffeur doit contenir au moins 2 caractères';
      return null;
    },
    vehicle: (value: unknown) => {
      const str = String(value);
      if (!str.trim()) return 'Le véhicule est requis';
      if (str.length < 2) return 'Le véhicule doit contenir au moins 2 caractères';
      return null;
    },
    estimatedDuration: (value: unknown) => {
      const num = Number(value);
      if (!num || num < 1) return 'La durée estimée doit être un nombre positif';
      return null;
    },
  ...
  const deliveryStats = useMemo(() => [
    ...
    {
      label: 'Programmées',
      value: deliveriesData.filter(d => d.status === 'scheduled').length,
      icon: Clock,
    ...
    {
      label: 'En Transit',
      value: deliveriesData.filter(d => d.status === 'in-transit').length,
      icon: Navigation,
    ...
    {
      label: 'Livrées',
      value: deliveriesData.filter(d => d.status === 'delivered').length,
      icon: CheckCircle,
  ...
  const partnerStats = useMemo(() => [
    ...
    {
      label: 'Actifs',
      value: partnersData.filter(p => p.status === 'active').length,
      icon: CheckCircle,
      color: 'text-green-600',
      bgColor: 'bg-green-100'
    },
    {
      label: 'Inactifs',
      value: partnersData.filter(p => p.status === 'inactive').length,
      icon: Clock,
      color: 'text-gray-600',
      bgColor: 'bg-gray-100'
    },
    {
      label: 'Suspendus',
      value: partnersData.filter(p => p.status === 'suspended').length,
      icon: XCircle,
      color: 'text-red-600',
      bgColor: 'bg-red-100'
    }
  ...
  const deliveryConfig = useMemo(() => ({
    ...
    description: 'Gérez les livraisons et les partenaires logistiques pour optimiser la chaîne d\'approvisionnement',
    icon: Truck,
    stats: deliveryStats,
  ...
  }), [deliveryStats, deliveryFormFields, deliveryViewFields, deliveryColumns, defaultDeliveryFormData, deliveryValidationRules]);
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-lg border border-gray-100">
            <div className="p-6 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900">Partenaires Transporteurs</h3>
              <p className="text-sm text-gray-600 mt-1">Gérez vos partenaires logistiques</p>
            </div>
            <div className="p-6">
              <DataTable
                data={partnersData}
                columns={partnerColumns as ColumnDef<LogisticsPartner, unknown>[]}
