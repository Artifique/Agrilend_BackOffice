...
import { 
  Shield, Download, Eye, Trash2, 
  CheckCircle, XCircle, AlertTriangle, Clock, Users,
  Calendar,
  Mail, Phone, User, Settings
} from 'lucide-react';
...
const mockGDPRUsers: GDPRUser[] = [
  {
    ...
    dataProcessingPurposes: ['service_provision', 'transaction_processing', 'communication'],
  ...
  },
  {
    ...
    dataProcessingPurposes: ['service_provision', 'transaction_processing', 'marketing'],
...
const GDPRCompliance: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      inactive: { color: 'bg-yellow-100 text-yellow-800', icon: Clock, text: 'Inactif' },
      deleted: { color: 'bg-red-100 text-red-800', icon: XCircle, text: 'Supprimé' }
    };
  ...
  const getConsentBadge = (consent: string) => {
    const consentConfig = {
      granted: { color: 'bg-green-100 text-green-800', icon: CheckCircle, text: 'Accordé' },
      denied: { color: 'bg-red-100 text-red-800', icon: XCircle, text: 'Refusé' },
      withdrawn: { color: 'bg-orange-100 text-orange-800', icon: AlertTriangle, text: 'Retiré' },
  ...
  const confirmConsentUpdate = (newConsent: string) => {
    if (selectedUser) {
      setGdprUsers(prev => prev.map(user => 
  ...
  const confirmDataExport = () => {
    if (selectedUser) {
      ...
      a.href = url;
      a.download = `gdpr-data-${selectedUser.userId}-${new Date().toISOString().split('T')[0]}.${exportFormat}`;
      a.click();
  ...
  const handleExport = () => {
    const data = filteredUsers.map(user => ({
      ...
      'Dernière mise à jour': new Date(user.lastConsentUpdate).toLocaleDateString('fr-FR'),
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `gdpr-compliance-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    window.URL.revokeObjectURL(url);
  };
  ...
  const columns: ColumnDef<GDPRUser>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Badge className={row.getValue('thirdPartySharing') ? 'bg-orange-100 text-orange-800' : 'bg-green-100 text-green-800'}>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-sm">
          {new Date(row.getValue('lastConsentUpdate')).toLocaleDateString('fr-FR')}
        </div>
      )
    },
    {
  ...
  const stats = useMemo(() => {
    ...
    const consentGranted = gdprUsers.filter(u => u.consentStatus === 'granted').length;
    const consentWithdrawn = gdprUsers.filter(u => u.consentStatus === 'withdrawn').length;
    const totalExportRequests = gdprUsers.reduce((sum, u) => sum + u.dataExportRequests, 0);
    const totalDeletionRequests = gdprUsers.reduce((sum, u) => sum + u.deletionRequests, 0);
    
    return { total, active, consentGranted, consentWithdrawn, totalExportRequests, totalDeletionRequests };
  }, [gdprUsers]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Gestion des données personnelles et droits des utilisateurs</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Consentement Accordé</p>
              <p className="text-2xl font-bold text-green-600">{stats.consentGranted}</p>
            </div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Consentement Retiré</p>
              <p className="text-2xl font-bold text-orange-600">{stats.consentWithdrawn}</p>
            </div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Exports Demandés</p>
              <p className="text-2xl font-bold text-blue-600">{stats.totalExportRequests}</p>
            </div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Suppressions Demandées</p>
              <p className="text-2xl font-bold text-red-600">{stats.totalDeletionRequests}</p>
            </div>
        ...
        {selectedUser && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-gray-900">Statut RGPD</h3>
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className={selectedUser.thirdPartySharing ? 'bg-orange-100 text-orange-800' : 'bg-green-100 text-green-800'}>
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Rétention des Données</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(selectedUser.dataRetention).map(([key, value]) => (
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Finalités de Traitement</h3>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-blue-50 rounded-lg p-4">
                  <div className="text-sm font-medium text-blue-800 mb-1">Demandes d'export</div>
                  <div className="text-2xl font-bold text-blue-600">{selectedUser.dataExportRequests}</div>
                </div>
                <div className="bg-red-50 rounded-lg p-4">
                  <div className="text-sm font-medium text-red-800 mb-1">Demandes de suppression</div>
                  <div className="text-2xl font-bold text-red-600">{selectedUser.deletionRequests}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-blue-800">Gestion du Consentement</span>
              </div>
              <p className="text-sm text-blue-700">
                Gestion du consentement pour <strong>{selectedUser.fullName}</strong>.
                Le consentement actuel est : <strong>{selectedUser.consentStatus}</strong>
              </p>
            </div>
          ...
          <div className="space-y-4">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-green-800">Droit à la portabilité</span>
              </div>
              <p className="text-sm text-green-700">
                Export des données personnelles de <strong>{selectedUser.fullName}</strong>.
            ...
            <div>
              ...
              <div className="space-y-2">
                {['Informations personnelles', 'Historique des transactions', 'Données de localisation', 'Communications', 'Préférences'].map(item => (
          ...
          <div className="space-y-4">
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-red-800">Droit à l'effacement</span>
              </div>
              <p className="text-sm text-red-700">
                <strong>ATTENTION :</strong> Cette action supprimera définitivement toutes les données personnelles de <strong>{selectedUser.fullName}</strong>.
            ...
            <div>
              ...
              <select className="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-red-500 focus:border-transparent">
                ...
                <option value="data_no_longer_needed">Données plus nécessaires</option>
